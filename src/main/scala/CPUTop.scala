...
class CPUTop extends Module {
  ...
  val signExtension=Wire(UInt(32.W))

  signExtension:=Cat(Fill(16,0.U),programMemory.io.instructionRead(15,0))
  alu.io.oper2:=Mux(controlUnit.io.aluSrc,signExtension,registerFile.io.b)
  alu.io.sel:=controlUnit.io.aluOp

  // Reduce sign four mux output from 32 to 16 bits
  val signReduction=Wire(UInt(16.W))
  signReduction:=Mux(controlUnit.io.loadFromMemory,registerFile.io.a,Mux(controlUnit.io.aluSrc,signExtension,registerFile.io.b))

  // RegisterFile -> DataMemory
  dataMemory.io.address:=signReduction
